	...
	<?php endif; ?>
		...
		switch($input["input_type"]) :
			...
			<?php break; case 'memo':
				...
				<textarea name="<?php echo $input["name"]; ?>" <?php echo $inputid; ?> <?php echo $disabled_element; ?> class="site-tracking"><?php echo stripslashes($input_value); ?></textarea>
			<?php	break; case 'file': ?>
                <?php if(isset($input["sub_title"])) : $meta_key = $input["sub_title"]; else : $meta_key = "logo"; endif; ?>
                
				<label><?php _e(ucfirst($meta_key),'ocmx'); ?></label>
				...
				if ($attachments) : ?><h4><?php _e("Select from previously uploaded ".$meta_key."'s",'ocmx'); ?></h4><?php endif;?>                
			...
			<?php break; case 'icon': ?>
               <?php if(isset($input["sub_title"])) : $meta_key = $input["sub_title"]; else : $meta_key = "icon"; endif; ?>
				...
				<div class="iconize">
					<a href="<?php echo $input_value; ?>" class="std_link iconfinder" rel="lightbox" target="_blank" id="<?php echo $input["id"]; ?>_href" style="background: url('<?php _e($input["icon"]);?>') no-repeat center;"><span>Default Icon</span></a>
					<input type="button" id="clear_<?php echo $input["id"]; ?>" value="<?php _e("Clear Icon",'ocmx'); ?>" class="button iconremove" />
				</div>
			
					<label><?php _e($input["label"],'ocmx');?></label> <br />
	                <input type="text"  name="<?php echo $input["name"]; ?>" id="<?php echo $input["id"]; ?>_text" value="<?php echo $input_value; ?>" />            
	                <input type="button"name="<?php echo $input["name"]; ?>_file" <?php echo $inputid; ?> value="<?php _e("Browse", 'ocmx'); ?>" class="button" />              
			...
			<?php  break; default :
				...
				<input type="text" name="<?php echo $input["name"]; ?>" <?php echo $inputid; ?> value="<?php echo $input_value; ?>" <?php echo $disabled_element; ?> />
			<?php break;
		endswitch; ?>
	<?php if($label_class !== "") : ?>
		</div>
